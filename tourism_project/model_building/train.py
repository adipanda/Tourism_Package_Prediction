...
mlflow.set_tracking_uri("http://127.0.0.1:5000")   # local MLflow server started by the GitHub Actions workflow
mlflow.set_experiment("TourismPredict")             # same experiment name as the dev experimentation cell
...
param_grid = {
    ...
    'xgbclassifier__reg_lambda': [0.5, 1.0],           # L2 regularization strength. Higher values help reduce overfitting.
}
# Model pipeline
model_pipeline = make_pipeline(preprocessor, xgb_model)   # chain the preprocessor and the XGBoost model
...
with mlflow.start_run():
    ...
    classification_threshold = 0.45    # tuned below 0.5 to favour recall on the minority (purchaser) class
    ...
    model_path = "tourism_project/deployment/best_tourism_package_model_v1.joblib"   # local path (inside tourism_project/deployment/) where the trained model is saved
    joblib.dump(best_model, model_path)  # save the best model to disk
